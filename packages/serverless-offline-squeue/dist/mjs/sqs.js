...
import { pipe, get, values, matches, find, mapValues, isPlainObject, toString, } from "lodash";
...
class SQS {
    ...
    _create(functionKey, rawSqsEventDefinition) {
        const sqsEvent = new SQSEventDefinition(rawSqsEventDefinition, this.options.region, this.options.accountId);
    ...
    async _sqsEvent(functionKey, sqsEvent) {
        ...
        const QueueUrl = this._rewriteQueueUrl((await this.client.getQueueUrl({ QueueName: queueName }).promise())
            .QueueUrl);
        const job = async () => {
    ...
    async _createQueue({ queueName }, remainingTry = 5) {
        try {
            ...
            await this.client
                .createQueue({
                QueueName: queueName,
                Attributes: mapValues((value) => isPlainObject(value) ? JSON.stringify(value) : toString(value), properties),
            })
                .promise();
        }
        catch (err) {
